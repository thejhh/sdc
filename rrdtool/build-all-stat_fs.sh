...
prefix="public/images/$(date -d "$(date "+$date 12:00:00")" "+%Y%m%d")-stat_fs-"

# Build stat_fs items for system interface
function build () {
	system="$1"
	filename="$2"
	start="$3"
	...
	./build-stat_fs.sh "$system" "$filename" free_blocks block_size "$start" "$end" "$prefix" "b" "B"
	#./build-stat_fs.sh "$system" memfree_kb "$start" "$end" "$prefix" "kb" "kB"
	#./build-stat_fs.sh "$system" buffers_kb "$start" "$end" "$prefix" "kb" "kB"
	#./build-stat_fs.sh "$system" cached_kb "$start" "$end" "$prefix" "kb" "kB"
